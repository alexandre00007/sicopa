...
def _seed(db):
    with db.connect() as con:
        ...
        """)
        rows = [
            ('p1','e1','i1','REGIME_A','raw_a','000123','MULUMBA','ALEXANDRE',1),
            ('p2','e2','i2','REGIME_B','raw_b','123','ALEXANDRE','MULUMBA',1),
        ]
        for line_id, execution_id, inst, regime, table_source, mat, nom, prenom, line_no in rows:
            con.execute("""INSERT INTO paie_standardisee
                ...
                [line_id,execution_id,inst,regime,'T1',2026,table_source,mat,mat,nom,prenom,
                 f'{nom} {prenom}','SEC','CAT','GR','UNIT','PROV',100,0,0,0,0,0,0,100,100,line_no])


def test_annex12_has_exactly_seven_linked_sheets(tmp_path):
    ...
    assert global_ws['E2'].hyperlink is not None
    ...
    assert detail_nom.max_row == 4  # retour + entete + 2 occurrences
