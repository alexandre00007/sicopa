...
class MatchingService:
    ...
    def run(self, institution_id: str, regime: str, quarter: str, year: int,
        ...
        formulas={rubric:self.db.impact_sql(institution_id,regime,quarter,year,rubric,"","rn_mat",
            formula_id=impact_formula_id) for rubric in rubrics}
        ...
        with self.db.connect() as con:
            ...
            con.execute("BEGIN")
            try:
                con.execute("DELETE FROM resultats_rapprochement WHERE institution_id=? AND regime=? AND trimestre=? AND annee=? AND statut_validation='A_VALIDER'",params)
                progress and progress(35,"Indexation des identifiants déclaratifs")
                con.execute(f"""
                    ...
                    d_mat AS (
                        SELECT matricule_normalise,MIN(ligne_declaratif_id) by_mat
                    ...
                    d_name AS (
                        SELECT nom_normalise,MIN(ligne_declaratif_id) by_name
                        FROM declaratif_standardise
                        WHERE institution_id=? AND regime=? AND trimestre=? AND annee=?
                          AND nom_normalise<>''
                    ...
                    SELECT uuid(),?,institution_id,regime,trimestre,annee,ligne_paie_id,COALESCE(by_mat,by_name),
                        ...
                        {mass_expr},{impact_case},0,'A_VALIDER',NULL,NULL,NULL,{formula_case}
                    FROM classified
                """,params+filter_params+params+params+[execution_id])
                progress and progress(85,"Identification des déclarés non payés")
