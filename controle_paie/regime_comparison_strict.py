    ...
    def run(self, institution_a: str, regime_a: str, institution_b: str, regime_b: str,
        ...
        with self.db.connect() as con:
            ...
            try:
                ...
                progress and progress(35, "Détection des collisions d'identité")
                con.execute("""CREATE OR REPLACE TEMP TABLE cmp_b_counts AS
                    SELECT matricule_normalise,
                      COUNT(*) FILTER (WHERE COALESCE(matricule_normalise,'') NOT IN ('','NU')) OVER (PARTITION BY matricule_normalise) n_mat,
                      COUNT(*) FILTER (WHERE COALESCE(nom_normalise,'')<>'') OVER (PARTITION BY nom_normalise) n_nom,
                      * EXCLUDE (matricule_normalise)
                    FROM cmp_b""")

                # Une ligne A produit au plus un match certain. Les cas ambigus ne choisissent jamais un B arbitraire.
                progress and progress(50, "Matching strict des identités")
                    ...
                    WITH af AS (
                      ...
                      SELECT af.*,
                        CASE
                          ...
                          WHEN COALESCE(matricule_normalise,'') IN ('','NU') AND nb_nom>1 THEN 'AMBIGU_NOM'
                          WHEN COALESCE(matricule_normalise,'') IN ('','NU') AND nb_nom=1 THEN 'NOM_PROBABLE'
                ...
                con.execute("""INSERT INTO resultats_comparaison_regimes
                    SELECT uuid(),?,
                      ...
                      p.occurrences,CASE WHEN p.match_type IN ('EXACT','NOM_PROBABLE','MATRICULE_INCOHERENT') THEN COALESCE(p.b_occurrences,0) ELSE 0 END,
                ...
                # Ajoute les B non consommés par un match exact/probable/incohérent unique.
                con.execute("""INSERT INTO resultats_comparaison_regimes
                    SELECT uuid(),?,'AUCUNE',
                    ...
                    WHERE NOT EXISTS (
                      ...
                      WHERE (p.match_type='EXACT' AND p.matricule_normalise=b.matricule_normalise AND p.nom_normalise=b.nom_normalise)
                         ...
                         OR (p.match_type='NOM_PROBABLE' AND p.nom_normalise=b.nom_normalise)
                    )""", [comparison_id])
                ...
                con.execute("""UPDATE resultats_comparaison_regimes SET
                    ecart_financier = CASE WHEN cle_type IN ('MATRICULE+NOM','NOM') AND occurrences_a>0 AND occurrences_b>0
                      AND statut NOT IN ('MATCH_AMBIGU_MATRICULE','MATCH_AMBIGU_NOM','IDENTITE_INCOHERENTE')
                      THEN ABS(ecart_remuneration)>=? OR ABS(ecart_net)>=? OR ABS(ecart_pourcentage)>=? ELSE FALSE END,
                    ecart_administratif = CASE WHEN cle_type IN ('MATRICULE+NOM','NOM') AND occurrences_a>0 AND occurrences_b>0
                      AND statut NOT IN ('MATCH_AMBIGU_MATRICULE','MATCH_AMBIGU_NOM','IDENTITE_INCOHERENTE')
                ...
                # Seul un match EXACT peut produire les statuts financiers/administratifs certains.
                con.execute("""UPDATE resultats_comparaison_regimes SET
                    statut=CASE
                ...
                # Statut dédié au potentiel double paiement uniquement lorsque le match exact n'a pas déjà une anomalie prioritaire.
                ...
                summary = con.execute("""SELECT
                    SUM(CASE WHEN occurrences_a>0 AND occurrences_b>0 AND cle_type='MATRICULE+NOM'
                      AND statut NOT IN ('MATCH_AMBIGU_MATRICULE','MATCH_AMBIGU_NOM','IDENTITE_INCOHERENTE') THEN 1 ELSE 0 END),
