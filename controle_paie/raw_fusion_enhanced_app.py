...
class PayrollAppWithEnhancedRawFusion(PayrollAppWithFlexibleAccess):
    ...
    def _enhance_raw_fusion_controls(self):
        ...
        # Ajoute un bouton Réanalyser près des actions de résultats.
        combo = self._find_filter_combo(self)
        if combo is not None:
            ttk.Button(combo.master, text="Réanalyser", style="Secondary.TButton",
                       command=self._reanalyze_current_raw_fusion).pack(side="right", padx=4)
    ...
    def _run_raw_fusion(self):
        ...
        self._open_raw_loader("Fusion & analyse multi-régimes",
                              f"{len(tables)} tables RAW • {quarter} {year}\nFusion, analyse, doublons et matrice s'exécutent en arrière-plan.")
        self._background(lambda: self.raw_fusion_service.create_fusion(tables, quarter, int(year), suffix, progress=self._progress),
                         self._raw_fusion_completed_with_loader, operation="Fusion RAW multi-régimes")
    ...
    def _reanalyze_current_raw_fusion(self):
        if not getattr(self, "raw_fusion_last_id", ""):
            messagebox.showwarning("Réanalyse", "Lancez ou rouvrez d'abord une fusion."); return
        info = self.raw_fusion_service.get_fusion(self.raw_fusion_last_id)
        if not messagebox.askyesno("Réanalyser", f"Recalculer toutes les analyses de {info['table']} ?\n\nLes résultats existants seront remplacés, mais la table RAW fusionnée restera intacte."):
            return
        self._open_raw_loader("Réanalyse multi-régimes",
                              f"Table : {info['table']}\nRecalcul des catégories, doublons, masses et diagnostics.")
        self._background(lambda: self.raw_fusion_service.reanalyze(self.raw_fusion_last_id, progress=self._progress),
                         self._raw_reanalysis_completed, operation="Réanalyse fusion multi-régimes")

    def _raw_reanalysis_completed(self, info):
        self._refresh_raw_fusion_summary(); self._refresh_raw_fusion_results()
        self._finish_raw_loader("Réanalyse terminée", "les résultats ont été recalculés")
        messagebox.showinfo("Réanalyse", "La fusion a été réanalysée avec les catégories et doublons mis à jour.")
    ...
    def _export_raw_fusion(self):
        ...
        info = self.raw_fusion_service.get_fusion(self.raw_fusion_last_id)
        self._open_raw_loader("Export des analyses multi-régimes",
                              f"Table : {info['table']}\nCréation de la synthèse, des annexes, doublons et matrice.")
        self._background(lambda: self.raw_fusion_service.export_all(self.raw_fusion_last_id, folder, progress=self._progress),
                         self._raw_export_completed, operation="Export fusion multi-régimes")
    ...
    def _show_raw_fusion_history(self):
        ...
        for col,title,width in [("table","Table fusionnée",260),("period","Période",90),("status","État",90),
        ...
        def reopen():
            fid = selected_id()
            if not fid: return
            self.raw_fusion_last_id = fid; self.raw_fusion_filter.set("Tous")
            self._refresh_raw_fusion_summary(); self._refresh_raw_fusion_results(); win.destroy()

        def reanalyze():
            fid = selected_id()
            if not fid: return
            self.raw_fusion_last_id = fid; info = self.raw_fusion_service.get_fusion(fid)
            if not messagebox.askyesno("Réanalyser", f"Réanalyser {info['table']} ?"): return
            win.destroy()
            self._open_raw_loader("Réanalyse multi-régimes", f"Table : {info['table']}\nRecalcul complet en cours.")
            self._background(lambda: self.raw_fusion_service.reanalyze(fid, progress=self._progress),
                             self._raw_reanalysis_completed, operation="Réanalyse fusion multi-régimes")
        ...
        def delete():
            ...
            win.destroy()
            self._open_raw_loader("Suppression de la fusion", f"Table : {info['table']}\nSuppression sécurisée des résultats et de la table fusionnée.")
            self._background(lambda: self._delete_raw_fusion_background(fid), self._raw_fusion_deleted,
