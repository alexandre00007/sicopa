...
class PayrollAppWithSqlTemplates(PayrollAppWithSqlConsole):
    """Console SQL enrichie de modèles et d'une coloration syntaxique."""
    ...
    def _install_sql_syntax_highlighting(self):
        ...
        def schedule(_event=None):
            if self._sql_highlight_after is not None:
                try:
                    self.after_cancel(self._sql_highlight_after)
                except Exception:
                    pass
    ...
    def _highlight_sql_editor(self):
        self._sql_highlight_after = None
        try:
            SqlSyntaxHighlighter.highlight(self.sql_editor)
        except tk.TclError:
            pass
    ...
    def _install_sql_template_controls(self, parent):
        ...
        ttk.Label(actions, text="Table A = table RAW sélectionnée à gauche. Les noms génériques de colonnes sont à adapter au schéma réel.", style="PageHint.TLabel").pack(side="left")
        ttk.Button(actions, text="Ajouter à la requête", style="Secondary.TButton", command=lambda: self._insert_sql_template(replace=False)).pack(side="right", padx=(6, 0))
        ttk.Button(actions, text="Remplacer par le modèle", style="Primary.TButton", command=lambda: self._insert_sql_template(replace=True)).pack(side="right")
    ...
    def _refresh_sql_template_tables(self):
        if not hasattr(self, "sql_template_table_b_combo"): return
